...
export function pointer({ skill }) {
  ...
  return [
    ...
    "When the user asks to build, fix, automate, or ship something **repeatable and verifiable**, reach for a `.loop` file — gated by the four-condition test in [`AGENTS.md`](./AGENTS.md): does the task repeat? is \"done\" checkable? are the iterations affordable? can the loop verify itself? **All four hold → author the `.loop`** (interview the user for the goal / `done when` / gates first; start from `templates/` when one fits), then " + run + ". **Any condition fails → do the work directly** — one-off questions and trivial edits never need a loop.",
